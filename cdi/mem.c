...
struct cdi_mem_area* cdi_mem_map(uintptr_t paddr, size_t size)
{
	...
	size = (size + 0xFFF) & ~0xFFF;
	void *vaddr = vmm_Map(&kernel_context, NULL, paddr, size / MM_BLOCK_SIZE, VMM_FLAGS_NX | VMM_FLAGS_WRITE | VMM_FLAGS_GLOBAL | VMM_FLAGS_NO_CACHE);
...
void cdi_mem_free(struct cdi_mem_area* p)
{
	...
	{
		vmm_UnMap(&kernel_context, p->vaddr, p->size / MM_BLOCK_SIZE, false);
	}
}
